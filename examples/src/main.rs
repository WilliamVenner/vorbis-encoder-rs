...
fn main() {
	...
	let buffer_size = 14;
	let packets = pcm.len() >> buffer_size;
	for _ in 1..packets {
		let rem = pcm.split_off(1 << buffer_size);
		file.write(encoder.encode(&pcm).expect("Error in encoding.")
			.as_slice()).expect("Error in writing");
		pcm = rem;
	}
